...
module.exports = msgHandler = async (nakano = new Client(), message) => {
    try {
        ...
        const botNumber = await nakano.getHostNumber() + '@c.us'
        const blockNumber = await nakano.getBlockedIds()
        const ownerNumber = config.ownerBot
        const groupId = isGroupMsg ? chat.groupMetadata.id : ''
        const groupAdmins = isGroupMsg ? await nakano.getGroupAdmins(groupId) : ''
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.whatsapp.com)/gi))) {
                const valid = await nakano.inviteInfo(chats)
                if (valid) {
                    console.log(color('[KICK]', 'red'), color('Received a group link and it is a valid link!', 'yellow'))
                    await nakano.reply(from, ind.linkDetected(), id)
                    await nakano.removeParticipant(groupId, sender.id)
                } else {
        ...
        if (isGroupMsg && !isGroupAdmins && isBotGroupAdmins && isDetectorOn && !isOwner) {
            if (chats.match(new RegExp(/(https:\/\/chat.(?!whatsapp.com))/gi))) {
                ...
                await nakano.reply(from, 'Fake group link detected!', id)
                await nakano.removeParticipant(groupId, sender.id)
            }
        }
            ...
            if (isUrl(chats)) {
                ...
                isPorn(classify.hostname, async (err, status) => {
                    ...
                    if (status) {
                        ...
                        await nakano.reply(from, ind.linkNsfw(), id)
                        await nakano.removeParticipant(groupId, sender.id)
                    } else {
        ...
        if (isGroupMsg && isAutoStickerOn && isMedia && isImage && !isCmd) {
            ...
            await nakano.sendImageAsSticker(from, imageBase64)
        ...
        if (isGroupMsg) {
            for (let ment of mentionedJidList) {
                if (afk.checkAfkUser(ment, _afk)) {
                    ...
                    await nakano.reply(from, ind.afkMentioned(getReason, getTime), id)
                }
            }
            if (afk.checkAfkUser(sender.id, _afk) && !isCmd) {
                _afk.splice(afk.getAfkPosition(sender.id, _afk), 1)
                fs.writeFileSync('./database/user/afk.json', JSON.stringify(_afk))
                await nakano.sendText(from, ind.afkDone(pushname))
            }
        }
        ...
        /*
        if (chats == 'p') {
            if (!isGroupMsg) await nakano.reply(from, `Halo Kak, Untuk Memulai bot silahkan ketik ${prefix}menu`, id)
        }
        ...
        if (chats == 'assalamualaikum') {
            if (!isGroupMsg) await nakano.reply(from, `Waalaikumsalam , Halo Kak, Untuk Memulai bot silahkan ketik ${prefix}menu`, id)
        }
        ...
        */
        ...
        switch (command) {
            case 'antiporn': // Premium, chat VideFikri
                await nakano.reply(from, 'Premium Feature!\n\nContact: wa.me/6285692655520?text=Buy%20Anti%20Porn', id)
            break

            // Register by Slavyan
            case 'register':
                if (isRegistered) return await nakano.reply(from, ind.registeredAlready(), id)
                if (isGroupMsg) return await nakano.reply(from, ind.pcOnly(), id)
                if (!q.includes('|')) return await nakano.reply(from, ind.wrongFormat(), id)
                const namaUser = q.substring(0, q.indexOf('|') - 1)
                const umurUser = q.substring(q.lastIndexOf('|') + 2)
                const serialUser = createSerial(20)
                if (Number(umurUser) >= 40) return await nakano.reply(from, ind.ageOld(), id)
                register.addRegisteredUser(sender.id, namaUser, umurUser, time, serialUser, _registered)
                await nakano.reply(from, ind.registered(namaUser, umurUser, sender.id, time, serialUser), id)
                console.log(color('[REGISTER]'), color(time, 'yellow'), 'Name:', color(namaUser, 'cyan'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'))
            break

            // Level [BETA] by Slavyan
            case 'level':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isLevelingOn) return await nakano.reply(from, ind.levelingNotOn(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                const userLevel = level.getLevelingLevel(sender.id, _level)
                const userXp = level.getLevelingXp(sender.id, _level)
                const ppLink = await nakano.getProfilePicFromServer(sender.id)
                if (ppLink === undefined) {
                    var pepe = errorImg
                } else {
                    pepe = ppLink
                ...
                rank.build()
                    .then(async (buffer) => {
                        ...
                        await nakano.sendFile(from, `${sender.id}_card.png`, `${sender.id}_card.png`, '', id)
                        fs.unlinkSync(`${sender.id}_card.png`)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
                    })
            break
            case 'leaderboard':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                try {
                    ...
                    await nakano.reply(from, leaderboard, id)
                } catch (err) {
                    console.error(err)
                    await nakano.reply(from, ind.minimalDb(), id)
                }
            break
            ...
            case 'joox': // By Hafizh
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!q) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                ...
                cardJoox.build()
                    .then(async (buffer) => {
                        ...
                        await nakano.sendFile(from, `${sender.id}_joox.png`, 'joox.png', ind.joox(dataJoox.data), id)
                        fs.unlinkSync(`${sender.id}_joox.png`)
                        await nakano.sendFileFromUrl(from, dataJoox.data.result[0].linkMp3, 'joox.mp3', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'instadl':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('instagram.com')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                    ...
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.post.length; i++) {
                            if (result.post[i].type === 'image') {
                                await nakano.sendFileFromUrl(from, result.post[i].urlDownload, 'igpostdl.jpg', `*...:* *Instagram Downloader* *:...*\n\nUsername: ${result.owner_username}\nCaption: ${result.caption}`, id)
                            } else if (result.post[i].type === 'video') {
                                await nakano.sendFileFromUrl(from, result.post[i].urlDownload, 'igpostdl.mp4', `*...:* *Instagram Downloader* *:...*\n\nUsername: ${result.owner_username}\nCaption: ${result.caption}`, id)
                            }
                        }
            ...
            case 'facebook':
            case 'fb':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(pushname), id)
                if (!isUrl(url) && !url.includes('facebook.com')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                downloader.fb(url)
                    .then(async ({ result }) => {
                            await nakano.sendFileFromUrl(from, result.VideoUrl, 'videofb.mp4', '', id)
            ...
            case 'ytmp3':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('youtu.be')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                downloader.ytdl(url)
                    .then(async (res) => {
                        if (res.status === 'error') {
                            await nakano.reply(from, res.pesan, id)
                        } else if (Number(res.size.split(' MB')[0]) >= 30) {
                            await nakano.reply(from, ind.videoLimit(), id)
                        } else {
                            await nakano.sendFileFromUrl(from, res.thumbnail, `${res.title}.jpg`, ind.ytFound(res), id)
                            await nakano.sendFileFromUrl(from, res.url_audio, `${res.title}.mp3`, '', id)
                ...
                downloader.ytdl(url)
                    .then(async (res) => {
                        if (res.status === 'error') {
                            ...
                            await nakano.sendFileFromUrl(from, res.thumbnail, `${res.title}.jpg`, ind.ytFound(res), id)
                            await nakano.sendFileFromUrl(from, res.url_video, `${res.title}.mp4`, '', id)
                            console.log('Success sending YouTube video!')
                        }
                    })
                    .catch(async (err) => {
            ...
            case 'tiktokpic':
                ...
                try {
                    ...
                    if (tkt.data.error) return nakano.reply(from, tkt.data.error, id)
                    await nakano.sendFileFromUrl(from, tkt.data.result, 'tiktokpic.jpg', 'Ini :D', id)
            ...
            case 'tktnowm':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('tiktok.com')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                downloader.tikNoWm(url)
                    .then(async ({ result }) => {
                        await nakano.sendFileFromUrl(from, result.thumb, 'TiktokNoWM.jpg', `➸ *Username*: ${result.username}\n➸ *Caption*: ${result.caption}\n➸ *Uploaded on*: ${result.uploaded_on}\n\nSedang dikirim, sabar ya...`, id)
                        const responses = await fetch(result.link);
                        const buffer = await responses.buffer();
                        fs.writeFileSync(`./temp/${sender.id}_TikTokNoWm.mp4`, buffer)
                        await nakano.sendFile(from, `./temp/${sender.id}_TikTokNoWm.mp4`, `${sender.id}_TikTokNoWm.mp4`, '', id)
            ...
            case 'tiktok':
                ...
                downloader.tik(url)
                    .then(async ({ result })=> {
                        ...
                        await nakano.sendFile(from, `./temp/${sender.id}_TikTok.mp4`, `${sender.id}_TikTok.mp4`, '', id)
            ...
            case 'twt':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('twitter.com')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                downloader.tweet(url)
                    .then(async (data) => {
                        if (data.type === 'video') {
                            ...
                            await nakano.sendFileFromUrl(from, content[0].url, 'video.mp4', `Link HD: ${result}`, id)
                            ...
                            for (let i = 0; i < data.variants.length; i++) {
                                await nakano.sendFileFromUrl(from, data.variants[i], data.variants[i].split('/media/')[1], '', id)
                                .then(() => console.log('Success sending Twitter media!'))
                                .catch(async (err) => {
                                    console.error(err)
                                    await nakano.reply(from, 'Error!', id)
            ...
            case 'moddroid': // Chikaa Chantekkzz
                ...
                downloader.modroid(q)
                    .then(async ({ status, result }) => {
                        if (status !== 200) {
                            await nakano.reply(from, 'Not found.', id)
                        } else {
                            await nakano.sendFileFromUrl(from, result[0].image, 'ksk.jpg', `*「 MOD FOUND 」*\n\n➸ *APK*: ${result[0].title}\n\n➸ *Size*: ${result[0].size}\n➸ *Publisher*: ${result[0].publisher}\n➸ *Version*: ${result[0].latest_version}\n➸ *Genre*: ${result[0].genre}\n\n*Download link*\n${result[0].download}`, id)
            ...
            case 'linedl': // chikaa chantexxzz
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await nakano.reply(from, ind.pcOnly(), id)
                if (!isUrl(url) && !url.includes('store.line.me')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                downloader.line(url)
                    .then(async (res) => {
                        await nakano.sendFileFromUrl(from, res.thumb, 'line.png', `*「 LINE STICKER DOWNLOADER 」*\n\n➸ *Title*: ${res.title}\n➸ *Sticker type*: ${res.type}\n\n_Media sedang dikirim, mohon tunggu sebentar..._`, id)
                        for (let i = 0; i < res.sticker.length; i++) {
                            await nakano.sendStickerfromUrl(from, `${res.sticker[i]}`)
            ...
            case 'googlesearch':
                ...
                google({ 'query': q, 'no-display': true })
                    .then(async (results) => {
                        ...
                        await nakano.reply(from, txt, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'say':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.sendText(from, q)
            break
            case 'afk': // by Slavyan
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                if (isAfkOn) return await nakano.reply(from, ind.afkOnAlready(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const reason = q ? q : 'Nothing.'
                afk.addAfkUser(sender.id, time, reason, _afk)
                await nakano.reply(from, ind.afkOn(pushname, reason), id)
            break
            case 'lyric':
            case 'lirik':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                misc.lirik(q)
                    .then(async ({ result }) => {
                        if (result.code !== 200) return await nakano.reply(from, 'Not found.', id)
                        await nakano.reply(from, result.result, id)
            ...
            case 'shortlink':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url)) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const urlShort = await misc.shortener(url)
                await nakano.reply(from, ind.wait(), id)
                await nakano.reply(from, urlShort, id)
                console.log('Success!')
            break
            case 'wikipedia':
            case 'wiki':
                ...
                misc.wiki(q)
                    .then(async ({ result, status }) => {
                        if (status !== 200) {
                            return await nakano.reply(from, 'Not found.', id)
                        } else {
                            await nakano.reply(from, result, id)
                ...
                misc.wikien(q)
                    .then(async ( { result }) => {
                        if (result.status !== '200') {
                            ...
                            await nakano.reply(from, `➸ *PageId*: ${result.pageid}\n➸ *Title*: ${result.title}\n➸ *Result*: ${result.desc}`, id)
                        }
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case 'ttp': // CHIKAA CHANTEKKXXZZ
                ...
                misc.ttp(q)
                    .then(async (res) => {
                        await nakano.sendImageAsSticker(from, res.base64)
            ...
            case 'genshin':
                ...
                try {
                    ...
                    await nakano.sendFileFromUrl(from, character.image, `${character.title}.jpg`, `*「 GENSHIN IMPACT 」*\n\n*${character.title}*\n${character.description}\n➸ *Name*: ${character.name}\n➸ *Nation*: ${character.nation}\n➸ *Gender*: ${character.gender}\n➸ *Birthday*: ${character.birthday}\n➸ *Constellation*: ${character.constellation}\n➸ *Rarity*: ${character.rarity}\n➸ *Vision*: ${character.vision}\n➸ *Weapon*: ${character.weapon}\n\n${character.url}`)
                    console.log('Success sending Genshin Impact character!')
                } catch (err) {
                    console.error(err)
                    await nakano.reply(from, 'Error or character not found!', id)
                }
            break
            case 'instastory': // By: VideFrelan
            case 'igstory':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!q) return nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                misc.its(q)
                    .then(async ({ result }) => {
                        for (let i = 0; i < result.story.itemlist.length; i++) {
                            ...
                            await nakano.sendFileFromUrl(from, urlDownload, '', 'By: VideFrelan', id)
            ...
            case 'kbbi':
                ...
                misc.kbbi(q)
                    .then(async ({ result }) => {
                        await nakano.reply(from, result.hasil, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'linestiker':
                ...
                misc.linesticker()
                    .then(async ({ result }) => {
                        ...
                        await nakano.reply(from, lines, id)
                ...
                misc.jadwalSholat(q)
                    .then((data) => {
                        data.map(async ({isya, subuh, dzuhur, ashar, maghrib, terbit}) => {
                            ...
                            await nakano.reply(from, msg, id)
            ...
            case 'gempa':
                ...
                misc.bmkg()
                    .then(async ({ kedalaman, koordinat, lokasi, magnitude, map, potensi, waktu }) => {
                        ...
                        await nakano.sendFileFromUrl(from, map, 'gempa.jpg', teksInfo, id)
                ...
                misc.igStalk(q)
                    .then(async ({ graphql }) => {
                        if (graphql === undefined) {
                            ...
                            await nakano.sendFileFromUrl(from, profile_pic_url_hd, 'insta.jpg', text, id)
            ...
            case 'gsmarena':
                ...
                try {
                    misc.gsmarena(q)
                        .then(async ({ result }) => {
                            await nakano.sendFileFromUrl(from, result.image, `${result.title}.jpg`, ind.gsm(result), id)
            ...
            case 'resep':
                ...
                try {
                    misc.resep(q)
                        .then(async ({ result }) => {
                            await nakano.sendFileFromUrl(from, result.image, `${result.title}.jpg`, ind.receipt(result), id)
            ...
            case 'findstiker':
                ...
                try {
                    misc.sticker(q)
                        .then(async ({ result }) => {
                            if (result.response !== 200) return await nakano.reply(from, 'Not found!', id)
                            for (let i = 0; i < result.data.length; i++) {
                                await nakano.sendStickerfromUrl(from, result.data[i])
                            }
                            console.log('Success sending sticker!')
                        })
                } catch (err) {
                    console.error(err)
                    await nakano.reply(from, `Error!\n\n${err}`, id)
                }
            break
            case 'movie':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                misc.movie(q)
                    .then(async ({ result }) => {
                        ...
                        movies += '\n\nBy: VideFrelan'
                        await nakano.reply(from, movies, id)
            ...
            case 'cekongkir': // By: VideFrelan
                ...
                misc.ongkir(kurir, askot, tukot)
                    .then(async ({ result }) => {
                        ...
                        onkir += '\n\nBy: VideFrelan'
                        await nakano.reply(from, onkir, id)
                ...
                misc.distance(kotaAsal, kotaTujuan)
                    .then(async ({ result }) => {
                        if (result.response !== 200) {
                            ...
                            await nakano.reply(from, result.data, id)
                    ...
                    misc.ytSearch(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                ...
                                await nakano.sendFileFromUrl(from, image, `${title}.jpg`, ind.ytResult(urlyt, title, channel, duration, views), id)
            ...
            case 'tts':
                ...
                try {
                    ptt.save(`${speech}.mp3`, speech, async () => {
                        await nakano.sendPtt(from, `${speech}.mp3`, id)
            ...
            case 'tomp3': // by: Piyobot
                ...
                if ((isMedia && isVideo || isQuotedVideo)) {
                    ...
                    fs.writeFile(fileInputPath, mediaData, (err) => {
                        ...
                        ffmpeg(fileInputPath)
                            ...
                            .on('end', async () => {
                                ...
                                await nakano.sendFile(from, fileOutputPath, 'audio.mp3', '', id)
                    ...
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'playstore':
            case 'ps':
                    ...
                    misc.playstore(q)
                        .then(async ({ result }) => {
                            for (let i = 0; i < 5; i++) {
                                ...
                                await nakano.sendFileFromUrl(from, icon, `${title}.jpg`, ind.playstore(app_id, title, developer, description, price, free))
                            }
            ...
            case 'math':
                ...
                if (typeof mathjs.evaluate(q) !== 'number') {
                    await nakano.reply(from, ind.notNum(q), id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, `*「 MATH 」*\n\n${q} = ${mathjs.evaluate(q)}`, id)
                }
            break
            case 'shopee':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                    ...
                    misc.shopee(namaBarang, jumlahBarang)
                        .then(async ({ result }) => {
                            for (let i = 0; i < result.items.length; i++) {
                                ...
                                await nakano.sendFileFromUrl(from, image_cover, `${nama}.jpg`, ind.shopee(nama, harga, terjual, shop_location, description, link_product))
                            }
            ...
            case 'mutual':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await nakano.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, 'Looking for a partner...', id)
                await nakano.sendContact(from, register.getRegisteredRandomId(_registered))
                await nakano.sendText(from, `Partner found: 🙉\n*${prefix}next* — find a new partner`)
            break
            case 'next':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) return await nakano.reply(from, 'Command ini tidak bisa digunakan di dalam grup!', id)
            ...
            case 'tafsir':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (args.length === 0) return nakano.reply(from, `Untuk menampilkan ayat Al-Qur'an tertentu beserta tafsir dan terjemahannya\ngunakan ${prefix}tafsir surah ayat\n\nContoh: ${prefix}tafsir Al-Mulk 10`, id)
                await nakano.reply(from, ind.wait(), id)
                ...
                if (!isNaN(nomerSurah)) {
                    ...
                    await nakano.reply(from, pesan, id)
                }
            break
            case 'listsurah':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                misc.listSurah()
                    .then(async ({ result }) => {
                        ...
                        await nakano.reply(from, list, id)
            ...
            case 'surah':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (args.length !== 1) return await nakano.reply(from, ind.wrongFormat(), id)
                await nakano.reply(from, ind.wait(), id)
                misc.getSurah(args[0])
                    .then(async ({ result }) => {
                        await nakano.reply(from, `${result.surah}\n\n${result.quran}`, id)
            ...
            case 'asmaulhusna': // irham01
                ...
                nakano.sendFileFromUrl(from, 'https://i2.wp.com/seruni.id/wp-content/uploads/2016/09/Allah.png?resize=696%2C696&ssl=1', 'gambar.jpg', ind.asmaulHusna(assna), id)
            break
            case 'randomquran': // irham01
                ...
                await nakano.reply(from, ind.randomQuran(ranquran), id)
                await nakano.sendFileFromUrl(from, auquran, 'rquran.mp3', '', id)
            break
            case 'motivasi':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                ...
                misc.motivasi()
                    .then(async (body) => {
                        ...
                        await nakano.sendText(from, randomMotivasi)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'play':
                ...
                misc.ytPlay(q)
                    .then(async ({ result }) => {
                        if (Number(result.size.split(' MB')[0]) >= 10.0) return nakano.sendFileFromUrl(from, result.image, `${result.title}.jpg`, `Judul: ${result.title}\nSize: *${result.size}*\n\nGagal, Maksimal video size adalah *10MB*!`, id)
                        await nakano.sendFileFromUrl(from, result.image, `${result.title}.jpg`, ind.ytPlay(result), id)
                        const responses = await fetch(result.mp3);
                        const buffer = await responses.buffer(); 
                        await fs.writeFile(`./temp/${result.title}.mp3`, buffer)
                        await nakano.sendFile(from, `./temp/${result.title}.mp3`, `${result.title}`, id)
            ...
            case 'playv': // Alvio Adji Januar 
                ...
                if (getvid.data.status === false) {
                    await nakano.reply(from, getvid.data.message, id)
                } else if (Number(getvid.data.result.size.split('MB')[0]) >= 10.00) {
                    await nakano.reply(from, 'Maaf durasi musik sudah melebihi batas maksimal 10 MB!', id)
                } else {
                    await nakano.sendFileFromUrl(from, getvid.data.result.thumb, 'thumb.jpg', `Title: ${getvid.data.result.title}\n\n───────────⚪───────────\n(っ◔◡◔)っ\n───────────⚪───────────\n➥Size: ${getvid.data.result.size}\n➥Type: Mp4\n➥Link Download: ${getvid.data.result.link}\n\n*Mohon Tunggu Bot Akan Mengirim Video!*`, id)
                    await nakano.sendFileFromUrl(from, getvid.data.result.link, 'play.mp4' , '', id)
                }
            break
            case 'whois':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                misc.email(emailTarget, subjectEmail, messageEmail)
                    .then(async ({ result }) => {
                        if (result.status === '204') {
                            await nakano.reply(from, 'Server busy!', id)
                        } else {
                            await nakano.reply(from, `*Success sending email*!\n➸ *Target*: ${emailTarget}\n➸ *Subject*: ${result.subjek}\n➸ *Message*: ${result.pesan}`, id)
            ...
            case 'sms':
                ...
                misc.sms(nomorPenerima, pesanPengirim)
                    .then(async ({ status, pesan }) => {
                        if (status !== 'success') return await nakano.reply(from, pesan, id)
                        await nakano.reply(from, `Success sending SMS to: ${nomorPenerima}\nMessage: ${pesanPengirim}`, id)
            ...
            case 'toxic':
                if (!isRegistered) return await nakano.reply(from , ind.notRegistered(), id)
                await nakano.reply(from, toxic(), id)
            break
            case 'alkitab':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!q) return await nakano.reply(from, ind.wrongFormat(), id)
                ...
                misc.alkitab(q)
                    .then(async ({ result }) => {
                        ...
                        await nakano.reply(from, alkitab, id)
            ...
            case 'imgtourl':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await nakano.reply(from, linkImg, id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'infohoax':
                ...
                misc.infoHoax()
                    .then(async ({ result }) => {
                        ...
                        await nakano.sendFileFromUrl(from, result[0].image, 'hoax.jpg', txt, id)
                        console.log('Success sending info!')
                    })
                    .catch(async (err) => {
                        console.error(err)
            ...
            case 'spamcall':
                ...
                misc.call(q)
                    .then(async ({ result }) => {
                        await nakano.reply(from, result.logs, id)
            ...
            case 'spamsms':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (args.length !== 2) return await nakano.reply(from, ind.wrongFormat(), id)
                if (isNaN(Number(args[0])) && isNaN(Number(args[1]))) return await nakano.reply(from, ind.wrongFormat(), id)
                if (Number(args[1]) > 10) return await nakano.reply(from, 'Maximum 10 SMS.', id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                misc.spamsms(args[0], args[1])
                    .then(async ({ status, logs, msg }) => {
                        if (status !== 200) {
                            ...
                            await nakano.reply(from, logs, id)
            ...
            case 'trans':
                ...
                translate(texto, {to: languaget}).then(res => {nakano.reply(from, res.text, id)})
            break
            ...
            case 'help':
                ...
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (args[0] === '1') {
                    await nakano.sendText(from, ind.menuDownloader())
                } else if (args[0] === '2') {
                    await nakano.sendText(from, ind.menuBot())
                } else if (args[0] === '3') {
                    await nakano.sendText(from, ind.menuMisc())
                } else if (args[0] === '4') {
                    await nakano.sendText(from, ind.menuSticker())
                } else if (args[0] === '5') {
                    await nakano.sendText(from, ind.menuWeeaboo())
                } else if (args[0] === '6') {
                    await nakano.sendText(from, ind.menuFun())
                } else if (args[0] === '7') {
                    await nakano.sendText(from, ind.menuModeration())
                } else if (args[0] === '8') {
                    if (isGroupMsg && !isNsfw) return await nakano.reply(from, ind.notNsfw(), id)
                    await nakano.sendText(from, ind.menuNsfw())
                } else if (args[0] === '9') {
                    if (!isOwner) return await nakano.reply(from, ind.ownerOnly())
                    await nakano.sendText(from, ind.menuOwner())
                } else if (args[0] === '10') {
                    if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                    await nakano.sendText(from, ind.menuLeveling())
                } else {
                    await nakano.sendText(from, ind.menu(jumlahUser, levelMenu, xpMenu, role, pushname, reqXpMenu, isPremium ? 'YES' : 'NO'))
                }
            break
            case 'rules':
            case 'rule':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                await nakano.sendText(from, ind.rules())
            break
            case 'nsfw':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isNsfw) return await nakano.reply(from, ind.nsfwAlready(), id)
                    _nsfw.push(groupId)
                    fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(_nsfw))
                    await nakano.reply(from, ind.nsfwOn(), id)
                } else if (ar[0] === 'disable') {
                    _nsfw.splice(groupId, 1)
                    fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(_nsfw))
                    await nakano.reply(from, ind.nsfwOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'status':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                await nakano.sendText(from, `*RAM*: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB / ${Math.round(os.totalmem / 1024 / 1024)} MB\n*CPU*: ${os.cpus()[0].model}`)
            break
            case 'listblock':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                await nakano.sendTextWithMentions(from, block)
            break
            case 'ownerbot':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                await nakano.sendContact(from, ownerNumber)
            break
            case 'runtime': // BY HAFIZH
                ...
                await nakano.reply(from, `── *「 BOT UPTIME 」* ──\n\n ❏${formater(uptime)}`, id)
            break
            case 'ping':
            case 'p':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                await nakano.sendText(from, `Pong!\nSpeed: ${processTime(t, moment())} secs`)
            break
            case 'delete':
            case 'del':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!quotedMsg) return await nakano.reply(from, ind.wrongFormat(), id)
                if (!quotedMsgObj.fromMe) return await nakano.reply(from, ind.wrongFormat(), id)
                await nakano.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
            case 'report':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!q) return await nakano.reply(from, ind.emptyMess(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                if (lastReport !== undefined && cd - (Date.now() - lastReport) > 0) {
                    ...
                    await nakano.reply(from, ind.daily(time), id)
                } else {
                    if (isGroupMsg) {
                        await nakano.sendText(ownerNumber, `-----[ REPORT ]-----\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Group*: ${(name || formattedTitle)}\n*Message*: ${q}`)
                        await nakano.reply(from, ind.received(pushname), id)
                    } else {
                        await nakano.sendText(ownerNumber, `-----[ REPORT ]-----\n\n*From*: ${pushname}\n*ID*: ${sender.id}\n*Message*: ${q}`)
                        await nakano.reply(from, ind.received(pushname), id)
                    }
            ...
            case 'tos':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                await nakano.sendLinkWithAutoPreview(from, 'https://github.com/dxxoo/nakanoBOT-Whatsapp-Bot', ind.tos(ownerNumber))
            break
            case 'join':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('chat.whatsapp.com')) return await nakano.reply(from, ind.wrongFormat(), id)
                const checkInvite = await nakano.inviteInfo(url)
                if (isOwner) {
                    await nakano.joinGroupViaLink(url)
                    await nakano.reply(from, ind.ok(), id)
                    await nakano.sendText(checkInvite.id, `Hello!! I was invited by ${pushname}`)
                } else {
                    const getGroupData = await nakano.getAllGroups()
                    if (getGroupData.length >= groupLimit) {
                        await nakano.reply(from, `Invite refused. Max group is: ${groupLimit}`, id)
                    } else if (getGroupData.size <= memberLimit) {
                        await nakano.reply(from, `Invite refused. Minimum member is: ${memberLimit}`, id)
                    } else {
                        if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                        limit.addLimit(sender.id, _limit, isPremium, isOwner)
                        await nakano.joinGroupViaLink(url)
            ...
            case 'cekpremium':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isPremium) return await nakano.reply(from, ind.notPremium(), id)
                const cekExp = ms(premium.getPremiumExpired(sender.id, _premium) - Date.now())
                await nakano.reply(from, `*「 PREMIUM EXPIRE 」*\n\n➸ *ID*: ${sender.id}\n➸ *Premium left*: ${cekExp.days} day(s) ${cekExp.hours} hour(s) ${cekExp.minutes} minute(s)`, id)
            break
            case 'premiumlist':
            case 'listpremium':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                for (let i = 0; i < deret.length; i++) {
                    ...
                    arrayPremi.push(await nakano.getContact(premium.getAllPremiumUser(_premium)[i]))
                    listPremi += `${i + 1}. wa.me/${premium.getAllPremiumUser(_premium)[i].replace('@c.us', '')}\n➸ *Name*: ${arrayPremi[i].pushname}\n➸ *Expired*: ${checkExp.days} day(s) ${checkExp.hours} hour(s) ${checkExp.minutes} minute(s)\n\n`
                }
                await nakano.reply(from, listPremi, id)
            break
            case 'getpic':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (mentionedJidList.length !== 0) {
                    const userPic = await nakano.getProfilePicFromServer(mentionedJidList[0])
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    await nakano.sendFileFromUrl(from, pek, 'pic.jpg', '', id)
                } else if (args.length !== 0) {
                    const userPic = await nakano.getProfilePicFromServer(args[0] + '@c.us')
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    if (userPic === undefined) {
                        var peks = errorImg
                    ...
                    await nakano.sendFileFromUrl(from, peks, 'pic.jpg', '', id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'serial':
                if (!isRegistered) return await nakano.reply(from, ind.registered(), id)
                if (isGroupMsg) return await nakano.reply(from, ind.pcOnly(), id)
                if (args.length !== 1) return await nakano.reply(from, ind.wrongFormat(), id)
                ...
                if (register.checkRegisteredUserFromSerial(serials, _registered)) {
                    ...
                    await nakano.sendText(from, ind.registeredFound(name, age, time, serials, id))
                } else {
                    await nakano.sendText(from, ind.registeredNotFound(serials))
                }
            break
            case 'limit':
                if (isPremium || isOwner) return await nakano.reply(from, '⤞ Limit left: ∞ (UNLIMITED)', id)
                await nakano.reply(from, `⤞ Limit left: ${limit.getLimit(sender.id, _limit, limitCount)} / 25\n\n*_Limit direset pada pukul 00:00 WIB_*`, id)
            break
            ...
            case 'kelpersegi':
                ...
                await nakano.reply(from, `*Hasil*: ${persegi}\n*Rumus*: ${caraPersegi}`, id)
            break
            case 'luaspersegi':
                if (!isRegistered) return await nakano.reply(from, ind.registered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                ...
                await nakano.reply(from, `*Hasil*: ${luaspersegi}\n*Rumus*: ${luaspersegis}`, id)
            break
            case 'kuadrat':
                if (!isRegistered) return await nakano.reply(from, ind.registered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                ...
                await nakano.reply(from, `*Hasil*: ${kuadrat}`, id)
            break
            case 'kubik':
                if (!isRegistered) return await nakano.reply(from, ind.registered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                ...
                await nakano.reply(from, `*Hasil*: ${kubik}`, id)
            break
            ...
            case 'neko':
                ...
                await nakano.sendFileFromUrl(from, (await neko.sfw.neko()).url, 'neko.jpg', '', null, null, true)
            ...
            case 'wp':
                ...
                await nakano.sendFileFromUrl(from, (await neko.sfw.wallpaper()).url, 'wallpaper.jpg', '', null, null, true)
            ...
            case 'kemono':
                ...
                await nakano.sendFileFromUrl(from, (await neko.sfw.kemonomimi()).url, 'kemono.jpg', '', null, null, true)
            ...
            case 'kusonime':
                ...
                weeaboo.anime(q)
                    .then(async ({ info, link_dl, sinopsis, thumb, title, error, status }) => {
                        if (status === false) {
                            return await nakano.reply(from, error, id)
                        } else {
                            let animek = `${title}\n\n${info}\n\nSinopsis: ${sinopsis}\n\nLink download:\n${link_dl}`
                            await nakano.sendFileFromUrl(from, thumb, 'animek.jpg', animek, null, null, true)
            ...
            case 'komiku':
                ...
                weeaboo.manga(q)
                    .then(async ({ genre, info, link_dl, sinopsis, thumb }) => {
                        ...
                        await nakano.sendFileFromUrl(from, thumb, 'mangak.jpg', mangak, null, null, true)
                    ...
                    weeaboo.wait(imageBase64)
                        .then(async (result) => {
                            if (result.docs && result.docs.length <= 0) {
                                return await nakano.reply(from, 'Anime not found! :(', id)
                            } else {
                                ...
                                if (similarity < 0.92) {
                                    ...
                                    await nakano.sendFileFromUrl(from, video, `${title_romaji}.mp4`, teks, id)
                    ...
                    try {
                        ...
                        for (let i = 0; i < results.length; i++) {
                            ...
                            if (results[i].similarity < 80.00) {
                                ...
                                await nakano.sendLinkWithAutoPreview(from, results[i].url, teks)
            ...
            case 'waifu':
                ...
                weeaboo.waifu(false)
                    .then(async ({ url }) => {
                        await nakano.sendFileFromUrl(from, url, 'waifu.png', '', id)
            ...
            case 'anitoki':
                ...
                weeaboo.anitoki()
                    .then(async ({ result }) => {
                        ...
                        await nakano.reply(from, anitoki, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'neonime':
                ...
                weeaboo.neonime()
                    .then(async ({ status, result }) => {
                        if (status !== 200) return await nakano.reply(from, 'Not found.', id)
                        let neoInfo = '-----[ *NEONIME LATEST* ]-----'
                        for (let i = 0; i < result.length; i++) {
                            const { date, title, link, desc } = result[i]
                            neoInfo += `\n\n➸ *Title*: ${title}\n➸ *Date*: ${date}\n➸ *Synopsis*: ${desc}\n➸ *Link*: ${link}\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                        }
                        await nakano.reply(from, neoInfo, id)
            ...
            case 'anoboy':
                ...
                weeaboo.anoboy()
                    .then(async ({ result }) => {
                        ...
                        await nakano.reply(from, anoboyInfo, id)
            ...
            case 'animesticker': 
                ...
                weeaboo.snime()
                    .then(async (body) => {
                        ...
                        await nakano.sendStickerfromUrl(from, wifegerakx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'bapak': // By Kris
                ...
                axios.get(`https://api.terhambar.com/bpk?kata=${q}`)
                    .then(async (res) => await nakano.reply(from, res.data.text, id))
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
                    })
            ...
            case 'puisi': // By Kris
                ...
                axios.get('https://masgi.herokuapp.com/api/puisi2')
                    .then(async (res) => await nakano.reply(from, res.data.data, id))
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
                    })
            ...
            case 'creepyfact': // By Kris
                ...
                fetch('https://raw.githubusercontent.com/TheSploit/CreepyFact/main/creepy.txt')
                    ...
                    .then(async (body) => {
                        ...
                        await nakano.reply(from, creepyz, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'quotes':
                ...
                misc.quotes()
                .then(async ({ result }) => {
                    await nakano.reply(from, `➸ *Quotes*: ${result.quotes}\n➸ *Author*: ${result.author}`, id)
                })
            break
            case 'asupan': // shansekai
                ...
                fun.asupan()
                    .then(async (body) => {
                        ...
                        await nakano.sendFileFromUrl(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, 'asupan.mp4', 'Follow IG: https://www.instagram.com/ptl_repost untuk mendapatkan asupan lebih banyak.', id)
            ...
            case 'citacita': // Piyobot
                ...
                fun.cita()
                    .then(async (body) => {
                        ...
                        await nakano.sendFileFromUrl(from, randomCita, 'cita.mp3', '', id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'dadu': // by CHIKAA CHANTEKKXXZZ
                ...
                fun.dadu()
                    .then(async (body) => {
                        ...
                        await nakano.sendStickerfromUrl(from, dadugerakx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'doge':
                ...
                fun.doge()
                    .then(async (body) => {
                        ...
                        await nakano.sendStickerfromUrl(from, dogegx)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'me':
                ...
                if (quotedMsg) {
                    ...
                    const profilePic = await nakano.getProfilePicFromServer(getQuoted)
                    const username = quotedMsgObj.sender.name
                    const statuses = await nakano.getStatus(getQuoted)
                    ...
                    await nakano.sendFileFromUrl(from, pfp, `${username}.jpg`, ind.profile(username, status, premi, benet, adm, levelMe, req, xpMe), id)
                } else {
                    const profilePic = await nakano.getProfilePicFromServer(sender.id)
                    const username = pushname
                    const statuses = await nakano.getStatus(sender.id)
                    ...
                    await nakano.sendFileFromUrl(from, pfps, `${username}.jpg`, ind.profile(username, status, premi, benet, adm, levelMe, req, xpMe), id)
                }
            break
            case 'hartatahta':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/hartatahta?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case 'nulis':
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/write?text=${q}&apikey=${config.vhtear}`, 'nulis.jpg', '', id)
            ...
            case 'ffbanner': // By: VideFrelan
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/bannerff?title=${teks1ffanjg}&text=${teks2ffanjg}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case 'caklontong': //By: VideFrelan
                if (!isGroupMsg) return nakano.reply(from, ind.groupOnly(), id)
                if (!isRegistered) return  nakano.reply(from, ind.notRegistered(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                const sleep = (ms) => {
                ...
                fun.caklontong()
                    .then(async ( { result }) => {
                        await nakano.reply(from, `➸ *Soal*: ${result.soal}`, id)
                        await nakano.sendText(from, '30 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.sendText(from, '20 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.sendText(from, '10 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.reply(from, `➸ *Jawaban*: ${result.jawaban}\n${result.desk}`, id)
            ...
            case 'tebakgambar':
                ...
                fun.tbkgmbr()
                    .then(async ({ result }) => {
                        await nakano.sendFileFromUrl(from, result.soal_gbr, 'TebakGambar.jpg', '', id)
                        await nakano.sendText(from, '50 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.sendText(from, '40 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.sendText(from, '30 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.sendText(from, '20 Detik Tersisa...')
                        ...
                        await nakano.sendText(from, '10 Detik Tersisa...')
                        await sleep(10000)
                        await nakano.reply(from, `➸ *Jawaban*: ${result.jawaban}`, id)
            ...
            case 'fflogo': // By: VideFrelan
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/logoff?hero=${karakter}&text=${teksff}&apikey=${config.vhtear}`, id)
                console.log('Success!')
            break
            case 'text3d':
            case '3dtext':
                ...
                await nakano.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/text3d?text=${q}`,`${q}.jpg`, '', id)
            ...
            case 'simi': // by: VideFrelan
                ...
                fun.simi(q)
                    .then(async ({ jawaban }) => {
                        await nakano.reply(from, jawaban, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, `Error!\n\n${err}`, id)
            ...
            case 'glitext':
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/glitchtext?text1=${teks1}&text2=${teks2}&apikey=${config.vhtear}`, 'glitch.jpg', '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'phmaker':
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/pornlogo?text1=${kiri}&text2=${kanan}&apikey=${config.vhtear}`, 'ph.jpg', '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'blackpink':
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/blackpinkicon?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
            ...
            case 'galaxy':
                ...
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/galaxytext?text=${q}&apikey=${config.vhtear}`, `${q}.jpg`, '', id)
                    .then(() => console.log('Success creating image!'))
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'tod':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                await nakano.reply(from, 'Sebelum bermain berjanjilah akan melaksanakan apapun perintah yang diberikan.' , id)
                await nakano.sendText(from, `Silakan ketik *${prefix}truth* atau *${prefix}dare*`)
            break
            case 'weton':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!q.includes('|')) return await nakano.reply(from, ind.wrongFormat(), id)
            ...
            case 'truth':
                ...
                fun.truth()
                    .then(async (body) => {
                        ...
                        await nakano.reply(from, randomTod, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
            ...
            case 'hilih':
                ...
                fun.hilihteks(q)
                    .then(async ( { result }) => {
                        await nakano.reply(from, result.kata, id)
            ...
            case 'dare':
                ...
                fun.dare()
                    .then(async (body) => {
                        ...
                        await nakano.reply(from, randomDare, id)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
                    ...
                    canvas.Canvas.trigger(mediaData)
                        .then((buffer) => {
                            ...
                            ffmpeg(fileInputPath)
                                ...
                                .on('end', async () => {
                                    console.log(color('[FFmpeg]', 'green'), 'Processing finished!')
                                    await nakano.sendMp4AsSticker(from, fileOutputPath, { fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', loop: 0 })
            ...
            case 'trash':
                ...
                try {
                    ...
                    for (let i = 0; i < mentionedJidList.length; i++) {
                        const ypics = await nakano.getProfilePicFromServer(mentionedJidList[i])
                        if (ypics === undefined) {
                            var ypfps = errorImg
                        } else {
                            ypfps = ypics
                    ...
                    canvas.Canvas.trash(ypfps)
                        .then(async (buffer) => {
                            ...
                            await nakano.sendFile(from, `./temp/${sender.id}_trash.png`, `${sender.id}_trash.png`, '', id)
                ...
                try {
                    ...
                    canvas.Canvas.hitler(ypf)
                        .then(async (buffer) => {
                            ...
                            await nakano.sendFile(from, `./temp/${sender.id}_hitler.png`, `${sender.id}_hitler.png`, '', id)
            ...
            case 'wasted':
                ...
                if (isMedia && type === 'image' || isQuotedImage) {
                    ...
                    await nakano.reply(from, ind.wait(), id)
                    await nakano.sendFileFromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`, 'Wasted.jpg', 'Ini..., sticker nya lagi di kirim', id).then(() => nakano.sendStickerfromUrl(from, `https://some-random-api.ml/canvas/wasted?avatar=${fotoWtNya}`))
            ...
            case 'kiss':
                ...
                try {
                    if (isMedia && isImage || isQuotedImage) {
                        ...
                        await nakano.reply(from, ind.wait(), id)
                        const encryptMedia = isQuotedImage ? quotedMsg : message
                        const ppRaw = await nakano.getProfilePicFromServer(sender.id)
                        ...
                        canvas.Canvas.kiss(ppFirst, ppSecond)
                            .then(async (buffer) => {
                                ...
                                await nakano.sendFile(from, `${sender.id}_kiss.png`, `${sender.id}_kiss.png`, '', id)
            ...
            case 'phcomment':
                ...
                const ppPhRaw = await nakano.getProfilePicFromServer(sender.id)
                ...
                await nakano.sendFileFromUrl(from, preprocessPh.data.message, 'ph.jpg', '', id)
            ...
            case 'neon':
                ...
                await nakano.reply(from, ind.wait(), id)
                await nakano.sendFileFromUrl(from, `http://docs-jojo.herokuapp.com/api/neon?text1=${atasnya}&text2=${tengahnya}&text3=${bawahnya}`, 'neon.jpg', '', id)
                console.log('Success creating image!')
            break
            case 'firemaker':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/fire_maker?text=${q}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case 'mlmaker':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                await nakano.reply(from, ind.wait(), id)
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/logoml?hero=${namaHero}&text=${teksMl}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case 'balloonmaker':
            case 'blmaker':
                ...
                await nakano.reply(from, ind.wait(), id)
                await nakano.sendFileFromUrl(from, `https://api.vhtear.com/balloonmaker?text1=${namaKiri}&text2=${namaKanan}&apikey=${config.vhtear}`)
                console.log('Success creating image!')
            break
            case 'sliding':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                await nakano.sendVideoAsGif(from, `https://api.vhtear.com/slidingtext?text=${q}&apikey=${config.vhtear}`, 'sliding.gif', '', id)
            ...
            case 'stickernobg': //by: VideFrelan
                ...
                if (isMedia && type === 'image' || isQuotedImage) {
                    ...
                    misc.stickernobg(q)
                    .then(async ({ result }) => {
                        await nakano.sendStickerfromUrl(from, result.image)
                            ...
                            sharp(res)
                                ...
                                .toFile(`./temp/stage_${sender.id}.webp`, async (err) => {
                                    ...
                                    if (fs.existsSync(`./temp/${sender.id}.webp`)) {
                                        ...
                                        await nakano.sendRawWebpAsSticker(from, base64)
                                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
            ...
            case 'stikergif':
                ...
                if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                    ...
                    try {
                        ...
                        await nakano.sendMp4AsSticker(from, videoBase64, { fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', loop: 0 })
                            .then(async () => {
                                console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                                await nakano.sendText(from, ind.ok())
                            })
                    } catch (err) {
                        console.error(err)
                        await nakano.reply(from, ind.videoLimit(), id)
            ...
            case 'ttg':
                ...
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.reply(from, ind.wait(), id)
                await nakano.sendStickerfromUrl(from, `https://api.vhtear.com/textxgif?text=${q}&apikey=${config.vhtear}`)
            ...
            case 'toimg':
                ...
                if (isQuotedSticker) {
                    ...
                    try {
                        ...
                        await nakano.sendFile(from, imageBase64, 'sticker.jpg', '', id)
                    } catch (err) {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
                    }
            ...
            case 'emojistiker':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (args.length !== 1) return nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const emoji = emojiUnicode(args[0])
                await nakano.reply(from, ind.wait(), id)
                console.log('Creating emoji code for =>', emoji)
                await nakano.sendStickerfromUrl(from, `https://api.vhtear.com/emojitopng?code=${emoji}&apikey=${config.vhtear}`)
                    .then(async () => {
                        await nakano.reply(from, ind.ok(), id)
                        console.log(`Sticker processed for ${processTime(t, moment())} seconds`)
                    })
                    .catch(async (err) => {
                        console.error(err)
                        await nakano.reply(from, 'Emoji not supported!', id)
                    })
            break
            ...
            case 'mlewd':
                ...
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await nakano.reply(from, ind.notNsfw(), id)
                    if (!isPremium) return await nakano.reply(from, ind.notPremium(), id)
                    await nakano.reply(from, ind.botNotPremium(), id)
                } else {
                    if (!isPremium) return await nakano.reply(from, ind.notPremium(), id)
                    await nakano.reply(from, ind.botNotPremium(), id)
                }
            ...
            case 'lewd':
                ...
                if (isGroupMsg) {
                    ...
                    nsfw.randomLewd()
                        .then(async ({ url }) => {
                            await nakano.sendFileFromUrl(from, url, 'lewd.jpg', '', null, null, true)
            ...
            case 'fetish':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (ar.length !== 1) return await nakano.reply(from, ind.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await nakano.reply(from, ind.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    try {
                        if (ar[0] === 'armpits') {
                            nsfw.armpits()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'armpits.jpg', '', id)
                                        .then(() => console.log('Success sending armpits pic!'))
                                })
                        } else if (ar[0] === 'feets') {
                            nsfw.feets()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'feets.jpg', '', id)
                                        .then(() => console.log('Success sending feets pic!'))
                                })
                        } else if (ar[0] === 'thighs') {
                            nsfw.thighs()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'thighs.jpg', '', id)
                                        .then(() => console.log('Success sending thighs pic!'))
                                })
                        } else if (ar[0] === 'ass') {
                            nsfw.ass()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'ass.jpg', '', id)
                                        .then(() => console.log('Success sending ass pic!'))
                                })
                        } else if (ar[0] === 'boobs') {
                            nsfw.boobs()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'boobs.jpg', '', id)
                                        .then(() => console.log('Success sending boobs pic!'))
                                })
                        } else if (ar[0] === 'belly') {
                            nsfw.belly()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'belly.jpg', '', id)
                                        .then(() => console.log('Success sending belly pic!'))
                                })
                        } else if (ar[0] === 'sideboobs') {
                            nsfw.sideboobs()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'sideboobs.jpg', '', id)
                                        .then(() => console.log('Success sending sideboobs pic!'))
                                })
                        } else if (ar[0] === 'ahegao') {
                            nsfw.ahegao()
                                .then(async ({ url }) => {
                                    await nakano.sendFileFromUrl(from, url, 'ahegao.jpg', '', id)
                                        .then(() => console.log('Success sending ahegao pic!'))
                                })
                        } else {
                            await nakano.reply(from, 'Tag not found.', id)
                        }
                    } catch (err) {
                        console.error(err)
                        await nakano.reply(from, err, id)
                    }
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
            ...
            case 'nh':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (args.length !== 1) return await nakano.reply(from, ind.wrongFormat(), id)
                if (isNaN(Number(args[0]))) return await nakano.reply(from, ind.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await nakano.reply(from, ind.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    ...
                    if (validate === true) {
                        try {
                            ...
                            await nakano.sendFileFromUrl(from, pic, 'nhentai.jpg', teks, id)
                        ...
                        await nakano.reply(from, ind.nhFalse(), id)
                    }
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                ...
                if (isGroupMsg) {
                    ...
                    nana.search(q)
                        .then(async (g) => {
                            ...
                            await nakano.sendFileFromUrl(from, g.results[0].thumbnail.s, `${g.results[0].title}`, txt, id)
            ...
            case 'nekopoi':
                ...
                if (isGroupMsg) {
                    ...
                    try {
                        ...
                        await nakano.reply(from, text, id)
                    } catch (err) {
                        console.error(err)
                        await nakano.reply(from, 'Error!', id)
                    }
            ...
            case 'phdl':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isUrl(url) && !url.includes('pornhub.com')) return await nakano.reply(from, ind.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await nakano.reply(from, ind.notNsfw(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                        ...
                        nsfw.phDl(url)
                            .then(async ({ title, download_urls, thumbnail_url }) => {
                                ...
                                if (count !== 2) {
                                    ...
                                    await nakano.sendFileFromUrl(from, thumbnail_url, `${title}`, `Title: ${title}\n\nLinks:\n${shortsLow} (240P)\n${shortsMid} (480P)\n${shortsHigh} (720P)`, id)
            ...
            case 'yuri':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, ind.wait(), id)
                    await nakano.sendFileFromUrl(from, (await neko.nsfw.eroYuri()).url, 'yuri.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, ind.wait(), id)
            ...
            case 'lewdavatar':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, ind.wait(), id)
                    await nakano.sendFileFromUrl(from, (await neko.nsfw.avatar()).url, 'avatar.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, ind.wait(), id)
            ...
            case 'femdom':
                ...
                if (isGroupMsg) {
                    ...
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, ind.wait(), id)
                    await nakano.sendFileFromUrl(from, (await neko.nsfw.femdom()).url, 'femdom.jpg', '', id)
                } else {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.reply(from, ind.wait(), id)
            ...
            case 'cersex':
                ...
                if (isGroupMsg) {
                    ...
                    nsfw.cersex()
                        .then(async ({ result }) => {
                            await nakano.sendFileFromUrl(from, result.image, 'cersex.jpg', `── *「 ${result.judul} 」* ──\n\n${result.cerita}`, id)
            ...
            case 'revoke':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return nakano.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return nakano.reply(from, ind.botNotAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.revokeGroupInviteLink(groupId);
                nakano.sendTextWithMentions(from, `Group link revoked by @${sender.id.replace('@c.us', '')}`)
            break
            case 'linkgroup':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await nakano.reply(from, ind.botNotAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const gcLink = await nakano.getGroupInviteLink(groupId)
                const linkGc = `Group: *${formattedTitle}*\n\nLink: ${gcLink}`
                nakano.reply(from, linkGc, id)
            break
            case 'ownergroup':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                ...
                await nakano.sendTextWithMentions(from, `Owner Group : @${ownerGc}`)
            break
            case 'mutegc':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return nakano.reply(from, ind.groupOnly(), id)
                ...
                if (ar[0] === 'enable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.setGroupToAdminsOnly(groupId, true)
                    await nakano.sendText(from, ind.gcMute())
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.setGroupToAdminsOnly(groupId, false)
                    await nakano.sendText(from, ind.gcUnmute())
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'add':
                ...
                try {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await nakano.addParticipant(from, `${args[0]}@c.us`)
                    await nakano.sendText(from, '🎉 Welcome! 🎉')
                } catch (err) {
                    console.error(err)
                    await nakano.reply(from, 'Error!', id)
                }
            ...
            case 'kick':
                ...
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await nakano.reply(from, ind.botNotAdmin(), id)
                if (mentionedJidList.length === 0) return await nakano.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await nakano.reply(from, ind.wrongFormat(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.sendTextWithMentions(from, `Good bye~\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
                for (let i of mentionedJidList) {
                    if (groupAdmins.includes(i)) return await nakano.sendText(from, ind.wrongFormat())
                    await nakano.removeParticipant(groupId, i)
                }
            break
            case 'promote':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (!isBotGroupAdmins) return await nakano.reply(from, ind.botNotAdmin(), id)
                if (mentionedJidList.length !== 1) return await nakano.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await nakano.reply(from, ind.wrongFormat(), id)
                if (groupAdmins.includes(mentionedJidList[0])) return await nakano.reply(from, ind.adminAlready(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.promoteParticipant(groupId, mentionedJidList[0])
                await nakano.reply(from, ind.ok(), id)
            break
            case 'demote':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                if (mentionedJidList.length !== 1) return await nakano.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList[0] === botNumber) return await nakano.reply(from, ind.wrongFormat(), id)
                if (!groupAdmins.includes(mentionedJidList[0])) return await nakano.reply(from, ind.notAdmin(), id)
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                await nakano.demoteParticipant(groupId, mentionedJidList[0])
                await nakano.reply(from, ind.ok(), id)
            break
            case 'leave':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                if (!isGroupMsg) return await nakano.reply(from, ind.groupOnly(), id)
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                await nakano.sendText(from, 'Sayounara~ 👋')
                await nakano.leaveGroup(groupId)
            break
            case 'admins':
            case 'admin':
                if (!isRegistered) return await nakano.reply(from, ind.notRegistered(), id)
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const groupAdm = await nakano.getGroupAdmins(groupId)
                ...
                if (lastAdmin !== undefined && cd - (Date.now() - lastAdmin) > 0) {
                    ...
                    await nakano.sendTextWithMentions(from, txt)
                } else {
                    let txt = '╔══✪〘 *ADMINS* 〙✪══\n'
                    for (let i = 0; i < groupAdm.length; i++) {
                        txt += '╠➥'
            ...
            case 'everyone':
                ...
                if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                limit.addLimit(sender.id, _limit, isPremium, isOwner)
                const groupMem = await nakano.getGroupMembers(groupId)
            ...
            case 'groupicon':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    const mediaData = await decryptMedia(encryptMedia, uaOverride)
                    const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
                    await nakano.setGroupIcon(groupId, imageBase64)
                    await nakano.sendText(from, ind.ok())
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            ...
            case 'antilink':
                ...
                if (!isBotGroupAdmins) return await nakano.reply(from, ind.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isDetectorOn) return await nakano.reply(from, ind.detectorOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antilink.push(groupId)
                    fs.writeFileSync('./database/group/antilink.json', JSON.stringify(_antilink))
                    await nakano.reply(from, ind.detectorOn(name, formattedTitle), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antilink.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antilink.json', JSON.stringify(_antilink))
                    await nakano.reply(from, ind.detectorOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'leveling':
                ...
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isLevelingOn) return await nakano.reply(from, ind.levelingOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _leveling.push(groupId)
                    fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
                    await nakano.reply(from, ind.levelingOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _leveling.splice(groupId, 1)
                    fs.writeFileSync('./database/group/leveling.json', JSON.stringify(_leveling))
                    await nakano.reply(from, ind.levelingOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'welcome':
                ...
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isWelcomeOn) return await nakano.reply(from, ind.welcomeOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _welcome.push(groupId)
                    fs.writeFileSync('./database/group/welcome.json', JSON.stringify(_welcome))
                    await nakano.reply(from, ind.welcomeOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _welcome.splice(groupId, 1)
                    fs.writeFileSync('./database/group/welcome.json', JSON.stringify(_welcome))
                    await nakano.reply(from, ind.welcomeOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            ...
            case 'autostik':
                ...
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isAutoStickerOn) return await nakano.reply(from, ind.autoStikOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _autosticker.push(groupId)
                    fs.writeFileSync('./database/group/autosticker.json', JSON.stringify(_autosticker))
                    await nakano.reply(from, ind.autoStikOn(), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _autosticker.splice(groupId, 1)
                    fs.writeFileSync('./database/group/autosticker.json', JSON.stringify(_autosticker))
                    await nakano.reply(from, ind.autoStikOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'antinsfw':
                ...
                if (!isBotGroupAdmins) return await nakano.reply(from, ind.botNotAdmin(), id)
                if (ar[0] === 'enable') {
                    if (isDetectorOn) return await nakano.reply(from, ind.antiNsfwOnAlready(), id)
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antinsfw.push(groupId)
                    fs.writeFileSync('./database/group/antinsfw.json', JSON.stringify(_antinsfw))
                    await nakano.reply(from, ind.antiNsfwOn(name, formattedTitle), id)
                } else if (ar[0] === 'disable') {
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await nakano.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    _antinsfw.splice(groupId, 1)
                    fs.writeFileSync('./database/group/antinsfw.json', JSON.stringify(_antinsfw))
                    await nakano.reply(from, ind.antiNsfwOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            ...
            // Owner command
            case 'bc':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                if (!q) return await nakano.reply(from, ind.emptyMess(), id)
                const chats = await nakano.getAllChatIds()
                for (let bcs of chats) {
                    let cvk = await nakano.getChatById(bcs)
                    if (!cvk.isReadOnly) await nakano.sendText(bcs, `${q}\n\n- Slavyan (Kal)\n_Broadcasted message_`)
                }
                await nakano.reply(from, ind.doneOwner(), id)
            break
            case 'clearall':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                const allChats = await nakano.getAllChats()
                for (let delChats of allChats) {
                    await nakano.deleteChat(delChats.id)
                }
                await nakano.reply(from, ind.doneOwner(), id)
            break
            case 'leaveall':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                if (!q) return await nakano.reply(from, ind.emptyMess(), id)
                const allGroup = await nakano.getAllGroups()
                for (let gclist of allGroup) {
                    await nakano.sendText(gclist.contact.id, q)
                    await nakano.leaveGroup(gclist.contact.id)
                }
                await nakano.reply(from, ind.doneOwner())
            break
            case 'getses':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                const ses = await nakano.getSnapshot()
                await nakano.sendFile(from, ses, 'session.png', ind.doneOwner())
            break
            case 'ban':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                if (ar[0] === 'add') {
                    if (mentionedJidList.length !== 0) {
                        for (let benet of mentionedJidList) {
                            if (benet === botNumber) return await nakano.reply(from, ind.wrongFormat(), id)
                            _ban.push(benet)
            ...
            case 'ev':
                ...
                try {
                    ...
                    await nakano.sendText(from, evaled)
                } catch (err) {
                    console.error(err)
                    await nakano.reply(from, 'Error!', id)
                }
            ...
            case 'shutdown':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                await nakano.sendText(from, 'Otsukaresama deshita~ 👋')
                    .then(async () => await nakano.kill())
            ...
            case 'mute':
                ...
                if (!isGroupAdmins) return await nakano.reply(from, ind.adminOnly(), id)
                if (ar[0] === 'enable') {
                    if (isMute) return await nakano.reply(from, ind.muteChatOnAlready(), id)
                    _mute.push(groupId)
                    fs.writeFileSync('./database/bot/mute.json', JSON.stringify(_mute))
                    await nakano.reply(from, ind.muteChatOn(), id)
                } else if (ar[0] === 'disable') {
                    _mute.splice(groupId, 1)
                    fs.writeFileSync('./database/bot/mute.json', JSON.stringify(_mute))
                    await nakano.reply(from, ind.muteChatOff(), id)
                } else {
                    await nakano.reply(from, ind.wrongFormat(), id)
                }
            break
            case 'setname':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                if (!q || q.length > 25) return await nakano.reply(from, ind.wrongFormat(), id)
                await nakano.setMyName(q)
                await nakano.reply(from, `Done!\n\nUsername changed to: ${q}`, id)
            break
            case 'give':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                if (args.length !== 2) return await nakano.reply(from, ind.wrongFormat(), id)
                if (mentionedJidList.length !== 0) {
                    for (let give of mentionedJidList) {
                        ...
                        await nakano.reply(from, `Sukses menambah XP kepada: ${give}\nJumlah ditambahkan: ${args[1]}`, id)
                    }
                } else {
                    level.addLevelingXp(args[0] + '@c.us', Number(args[1]), _level)
                    await nakano.reply(from, `Sukses menambah XP kepada: ${args[0]}\nJumlah ditambahkan: ${args[1]}`, id)
                }
            break
            case 'listgroup':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
                    nakano.getAllGroups().then((res) => {
                    let gc = '*Group list*:\n'
                    for (let i = 0; i < res.length; i++) {
                        gc += `\n\n*No*: ${i+1}\n*Nama*: ${res[i].name}\n*Unread messages*: ${res[i].unreadCount} messages\n\n=_=_=_=_=_=_=_=_=_=_=_=_=`
                    }
                    nakano.reply(from, gc, id)
                })
            break
            case 'reset':
                if (!isOwner) return await nakano.reply(from, ind.ownerOnly(), id)
            ...
            default:
                if (isCmd) {
                    await nakano.reply(from, ind.cmdNotFound(command), id)
                }
            break
        }
    } catch (err) {
